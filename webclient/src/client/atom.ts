...
import { CAMERA_STATIC_PLANE, EMISSIVE_PLANE, EMISSIVE_RENDER_PLATE, LIGHTING_PLANE, O_LIGHTING_VISUAL_PLANE, WALL_PLANE } from "./planes";
	...
	get_render_plan(dt : number) {
		...
		if(!this.render_plan) {
			...
			if(this.vis_contents) for(let thing_id of this.vis_contents) {
				...
				if(thing_appearance.plane == O_LIGHTING_VISUAL_PLANE) {
			...
			if(appearance && appearance.invisibility <= this.client.eye_see_invisible) {
				...
				if(this.type == 1 && !tag?.length) {
					tag = appearance.plane == WALL_PLANE ? E3D_TYPE_SMOOTHWALL : E3D_TYPE_FLOOR;
				}
			...
			if(this.images) for(let image of this.images) {
				...
				if(!image_appearance || image_appearance.plane == CAMERA_STATIC_PLANE) continue;
...
let e3d_type_handlers = new Map<string, (this: Atom, list : RenderPlan[], appearance:Appearance, x:number, y:number) => void>(Object.entries({
	[E3D_TYPE_BILLBOARD](list, appearance, x, y) : void {
		...
		if(appearance.plane >= EMISSIVE_PLANE) plan.bits |= 1;
		if(appearance.overlays) for(let overlay of appearance.overlays) {
		...
		if(this.vis_contents) {
			for(let thing_id of this.vis_contents) {
				...
				if(thing_appearance.plane == 12 || thing_appearance.plane == O_LIGHTING_VISUAL_PLANE) continue;
				let plan;
				let bias = -0.051;
				...
				if(thing_appearance.plane == EMISSIVE_PLANE || thing_appearance.plane == EMISSIVE_RENDER_PLATE) {
					plan.icon |= (1 << 24);
					plan.bits |= 1;
	...
	[E3D_TYPE_FLOOR](list, appearance, x, y) : void {
		...
		if(vis_contents) for(let vc of vis_contents) {
			...
			if(!appearance || appearance.plane == O_LIGHTING_VISUAL_PLANE) continue;
	...
	[E3D_TYPE_WALLMOUNT](list, appearance, x, y) : void {
		...
		if(this.vis_contents) {
			for(let thing_id of this.vis_contents) {
				...
				if(thing_appearance.plane == EMISSIVE_PLANE || thing_appearance.plane == EMISSIVE_RENDER_PLATE || thing_appearance.plane > LIGHTING_PLANE) {
					plan.icon |= (1 << 24);
					plan.bits |= 1;
				}
			}
	...
	[E3D_TYPE_SMOOTHWALL](list, appearance, x, y) : void {
		...
		if(typeof smooths == "string") {
			...
			if(appearance.underlays) for(let underlay of appearance.underlays) {
				if(underlay.plane == LIGHTING_PLANE) continue;
	...
	[E3D_TYPE_DOOR](list, appearance, x, y) : void {
		...
		if(this.vis_contents) {
			for(let thing_id of this.vis_contents) {
				...
				if(thing_appearance.plane == O_LIGHTING_VISUAL_PLANE) continue;
				let plan;
