...
export function parse_appearance(dp : DataPointer, appearances : Map<number, Appearance>) {
	...
	if(!dp.reached_end()) {
		...
		if(exbits & 32) {
			appearance.maptext = dp.read_utf_string();
			appearance.maptext_width = dp.read_uint16();
			appearance.maptext_height = dp.read_uint16();
			appearance.maptext_x = dp.read_int16();
			appearance.maptext_y = dp.read_int16();
		}
		if(exbits & 64) {
