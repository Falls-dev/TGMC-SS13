...
import { FULLSCREEN_PLANE, LIGHTING_PLANE } from "./planes";
...
export class SvgUi {
	...
	update_atom(atom : Atom) {
		...
		if(appearance && on_screen && appearance.plane == FULLSCREEN_PLANE && (appearance.icon_state == "blindimageoverlay")) {
		...
		if(!appearance || !on_screen || !screen_loc || appearance.plane < 0 || appearance.plane == LIGHTING_PLANE) {
			if(elem) {
	...
	appearance_to_elem(appearance : Appearance, elem : HTMLElement, pixel_xw : number, pixel_yz : number) : boolean {
		...
		if(appearance.maptext) {
			let maptext_elem = document.createElement("div");
			maptext_elem.className = "maptext";
			maptext_elem.style.position = "absolute";
			maptext_elem.style.left = appearance.maptext_x + "px";
			maptext_elem.style.bottom = appearance.maptext_y + "px";
			maptext_elem.style.width = appearance.maptext_width + "px";
			maptext_elem.style.height = appearance.maptext_height + "px";
			maptext_elem.style.pointerEvents = "none";
			maptext_elem.innerHTML = appearance.maptext;
			elem.style.overflow = "visible";
			elem.appendChild(maptext_elem);
		}
