...
import { CAMERA_STATIC_PLANE } from "./planes";
...
export class ByondClient {
	...
	constructor() {
		...
		window.addEventListener("keydown", (e) => {
			this.handle_key(e, "keydown");
		});
		window.addEventListener("keyup", (e) => {
			this.handle_key(e, "keyup");
		})
		window.addEventListener("message", (e) => {
			if (e.data && e.data.type === "wcp-key") {
				this.handle_key({
					code: e.data.code,
					key: e.data.key,
					which: e.data.which,
					...
					preventDefault() {},
				}, e.data.kind === "keyup" ? "keyup" : "keydown");
				return;
			}
	...
	handle_message = (e: MessageEvent) => {
		...
		case 0: {
			...
			// 516 may certify before sending an empty type-26 key request.
	...
	resolve_control(control : string) : HTMLElement|null {
		...
		if(control.endsWith(".browser")) {
			let id = control.slice(0, -8);
			return document.querySelector(`#window-${CSS.escape(id)} #control-browser`)
	...
	image_changes(dp : DataPointer, is_update : boolean) {
		...
		if(image.loc && image.appearance?.plane != CAMERA_STATIC_PLANE) this.active_images.add(image);
	...
	handle_key(e: Pick<KeyboardEvent, "code"|"key"|"which"|"repeat"|"target"|"preventDefault">, kind: "keydown"|"keyup") {
		if((e.target as HTMLElement)?.closest?.("input,select,button,textarea") || this.has_quit) return;
		if(kind == "keydown" && e.code == "Tab") {
			if(document.pointerLockElement != this.gl_holder.canvas) {
				this.gl_holder.canvas.requestPointerLock();
			} else {
		...
		if(this.is_test_env && kind == "keydown") {
		...
		let key = this.js_event_to_byond(e);
		if(!key) return;
		if(kind == "keyup") {
		...
		if(!e.repeat) {
			for(let macro of this.macros.values()) {
				...
				if(name == key || name == "Any") {
					let cmd = macro.get("command") ?? "";
					this.command(cmd.replace(/\[\[\*\]\]/g, key));
				}
			}
	...
	js_event_to_byond(e: {code?: string, key?: string, which?: number}) {
		if(e.code) {
			if(e.code.startsWith("Key") && e.code.length == 4) return e.code.substring(3);
			if(e.code.startsWith("Digit") && e.code.length == 6) return e.code.substring(5);
			if(e.code.startsWith("Numpad") && e.code.length == 7) return "Numpad" + e.code.substring(6);
			if(e.code.startsWith("F") && e.code.length <= 3 && e.code !== "F") {
				const n = +e.code.substring(1);
				if(n >= 1 && n <= 12) return e.code;
			}
			switch(e.code) {
				...
				case "Home": return "Northwest";
				case "End": return "Southwest";
				case "PageUp": return "Northeast";
				case "PageDown": return "Southeast";
				case "Comma": return ",";
				case "Period": return ".";
				case "Minus": return "-";
				case "Equal": return "=";
				...
				case "Enter":
				case "NumpadEnter": return "Return";
				case "Tab": return "Tab";
			}
		}
		if(e.key && e.key.length == 1) return e.key.toUpperCase();
		return this.js_keycode_to_byond(e.which ?? 0);
	}
