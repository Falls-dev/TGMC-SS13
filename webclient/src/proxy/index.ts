...
async function upgrade_request(request : IncomingMessage, socket : Socket, head : Buffer) {
	try {
		...
		let [byond_host, byond_port] = config.byond_addr.split(":");
		const topicResult = await sendTopic({ // Tell webclient_patches to expect us
			host: byond_host,
			port: +byond_port,
		...
		if(topicResult) {
			console.log("webclient_login topic response:", topicResult);
		}
		let byond_connection = await new Promise<WebSocket>((resolve, reject) => {
			...
			ws.on('error', (err) => {
				const hint = (err as NodeJS.ErrnoException).code === 'ECONNREFUSED'
					? `Nothing is listening at ${config.byond_addr}. Start Dream Daemon first (bin\\server.cmd).`
					: `BYOND rejected the webclient websocket at ${config.byond_addr}. Enable webclient: Dream Daemon → World → Allow webclient, or restart via bin\\server.cmd (now passes -webclient).`;
		...
		wss.handleUpgrade(request, socket, head, (client : WebSocket) => {
			...
			client.on('message', (data:Buffer, isBinary) => {
				...
				if(msgtype == 1) {
					// Custom client sends the version/IP handshake as type 1.
					// Only drop the official webclient's browser-fill payload.
					if(data.includes(Buffer.from("<script>"))) {
						return;
					}
				} else if(msgtype == 26) {
...
app.use("/browse/:userHash", async (req, res, next) => {
	try {
		...
		let body = Buffer.from(await fetch_res.arrayBuffer());
		const contentType = (fetch_res.headers.get('content-type') || '').toLowerCase();
		const head = body.subarray(0, 96).toString('utf8').toLowerCase();
		const isHtml = contentType.includes('html') || head.includes('<!doctype') || head.includes('<html');
		if (isHtml) {
			const inject = Buffer.from('<script src="/res/browserfill.js"></script>');
			const html = body.toString('utf8');
			const headTag = html.match(/<head[^>]*>/i);
			if (headTag && headTag.index != null) {
				const at = headTag.index + headTag[0].length;
				body = Buffer.concat([Buffer.from(html.slice(0, at), 'utf8'), inject, Buffer.from(html.slice(at), 'utf8')]);
			} else {
				body = Buffer.concat([inject, body]);
			}
		}
		res.end(body);
	} catch(e) {
