# Webclient Patches (TGMC / BYOND 516)

Patched so login still works when vis_contents / dmi signatures no longer match BYOND 516.
The hub-cert hook is required; other byte patches are optional.
...
Copy `target/i686-pc-windows-gnu/release/webclient_patches.dll` next to the `.dmb`.
