...
fn init_module() -> Result<(), &'static str> {
	unsafe {
		...
		// Optional on BYOND 516+: these byte patterns moved. Login still works without them.
		if let Ok(short_ptr) = extract_function_call(sig_short, BYONDCORE) {
			let _ = replace_function_call(sig_write_flags_delete, BYONDCORE, short_ptr);
			let _ = replace_function_call(sig_write_flags_modify, BYONDCORE, short_ptr);
		}
		...
		if let Ok(check_webclient_cert) = sig_check_webclient_cert.scan_module(BYONDCORE) {
			// Full replacement: we never call the original hub cert routine.
			let src = check_webclient_cert as u32;
			let dst = webclient_certificate_hook_platform as *const () as u32;
			let rel = dst.wrapping_sub(src.wrapping_add(5));
			let jmp = [0xE9, rel as u8, (rel >> 8) as u8, (rel >> 16) as u8, (rel >> 24) as u8];
			if replace_bytes(check_webclient_cert as *mut u8, &jmp).is_err() {
				undo_replacements();
				return Err("Couldn't patch check_webclient_cert");
			}
		} else {
			undo_replacements();
		...
		#[cfg(windows)] {
			...
			if let Ok(obj_ptr) = signature!(
				...
				let _ = replace_bytes(obj_ptr.offset(22), &[0x5a, 0x90, 0x85, 0xff, 0x74, 0x43, 0x52])
			...
			if let Ok(mob_ptr) = signature!(
				...
				let _ = replace_bytes(mob_ptr.offset(23), &[0x5a, 0x90, 0x85, 0xff, 0x74, 0x43, 0x52])
			...
			if let Ok(frame_loop_ptr) = signature!(
				...
				let _ = replace_bytes(frame_loop_ptr, &[0x90, 0xe9]);
			}
		}
		#[cfg(unix)] {
			...
			if let Ok(obj_ptr) = signature!(
				...
				let _ = replace_bytes(obj_ptr.offset(0), &id_saver_bytes)
			...
			if let Ok(mob_ptr) = signature!(
				...
				let _ = replace_bytes(mob_ptr.offset(0), &id_saver_bytes)
...
unsafe fn webclient_certificate_hook(token:*const c_char,callback : extern "C" fn(bool, *const c_char, u16)->(), client_id:u16) {
	if token.is_null() {
		callback(false, byond_return(Some("Auth failed".as_bytes().to_vec())), client_id);
		return;
	}
