...
function Details({ serverState }: { serverState: ServerState }) {
  return (
    <div className="escape-menu__details">
      <div>ID раунда: {serverState.roundId || 'ДЕБАГ'}</div>
      <div>Время сервера: {serverState.serverTime}</div>
      <div>Время операции: {serverState.shiftTime}</div>
      <div>Корабль: {serverState.shipMapName || 'Загрузка...'}</div>
      <div>Режим: {serverState.gameModeName || 'Загрузка...'}</div>
      <div>
        Карта:{' '}
      ...
      <div>Нагрузка: {serverState.timeDilation}%</div>
    </div>
  );
}
