...
/obj/machinery/self_destruct/console/ui_act(action, list/params)
	...
	switch(action)
		if("dest_start")
			...
			priority_announce("Тревога! Протокол самоуничтожения активирован. Корабль взорвется через 20 минут. Автоматическая детонация недоступна. Требуется ручное вмешательство.", title = "Протокол Самоуничтожения", type = ANNOUNCEMENT_PRIORITY, sound = 'modular_unga/ru_translate/ru_announce/sound/selfdestruct.ogg', color_override = "purple")
