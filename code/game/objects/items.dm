...
/obj/item/throw_impact(atom/hit_atom, speed, bounce)
	if(isturf(hit_atom) && !hit_atom.density)
		INVOKE_ASYNC(src, PROC_REF(roll_after_throw), dir)
	. = ..()
	if(. && isliving(hit_atom))
		playsound(src, hitsound, 50)

/obj/item/animation_spin(speed = 5, loop_amount = -1, clockwise = TRUE, sections = 3, anim_flags = NONE, angular_offset = 0, pixel_fuzz = 0)
	if(dir & WEST)
		clockwise = FALSE
	else if(dir & EAST)
		clockwise = TRUE
...
/obj/item/proc/roll_after_throw(roll_dir)
	if(!roll_dir)
		return

	var/roll_distance = 10
	var/dx = 0
	var/dy = 0
	if(roll_dir & NORTH) dy += roll_distance
	if(roll_dir & SOUTH) dy -= roll_distance
	if(roll_dir & EAST)  dx += roll_distance
	if(roll_dir & WEST)  dx -= roll_distance

	animate(src, pixel_x = pixel_x + dx, pixel_y = pixel_y + dy, time = 7.5, flags = ANIMATION_PARALLEL)
	sleep(7.5)
