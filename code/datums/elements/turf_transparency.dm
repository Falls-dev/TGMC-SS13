...
/datum/element/turf_z_transparency/proc/update_multi_z(turf/our_turf)
	...
	if(isclosedturf(our_turf)) //Show girders below closed turfs
		var/mutable_appearance/girder_underlay = mutable_appearance('modular_unga/features/wall_and_floor/icons/obj/smooth_objects/girder.dmi', "girder-0", layer = BELOW_CLOSED_TURF_LAYER) // tg uses a non smoothing one and we have a smoothing one, womp womp
