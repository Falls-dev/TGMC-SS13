...
/datum/scanner_advice/damage/brute/get_data(mob/living/carbon/human/patient, mob/user)
	...
	if(!(patient.species.species_flags & (IS_SYNTHETIC|ROBOTIC_LIMBS)))
		...
		if(patient.getBruteLoss() > 30 && !patient.reagents.has_reagent(/datum/reagent/medicalnanites) && !patient.reagents.has_reagent(/datum/reagent/medicine/bicaridine, 3) && !patient.reagents.has_reagent(/datum/reagent/medicine/sulfasalazine))
			. += list(list(
...
/datum/scanner_advice/damage/burn/get_data(mob/living/carbon/human/patient, mob/user)
	...
	if(!(patient.species.species_flags & (IS_SYNTHETIC|ROBOTIC_LIMBS)))
		...
		if(patient.getFireLoss() > 30 && !patient.reagents.has_reagent(/datum/reagent/medicalnanites) && !patient.reagents.has_reagent(/datum/reagent/medicine/kelotane, 3) && !patient.reagents.has_reagent(/datum/reagent/medicine/sulfasalazine))
			. += list(list(
...
/datum/scanner_advice/damage/tox/can_show(mob/living/carbon/human/patient, mob/user)
	...
	for(var/datum/reagent/reagent AS in patient.reagents.reagent_list)
		...
		if(istype(reagent, /datum/reagent/toxin) && !has_dylovene)
			return TRUE
	if(patient.getToxLoss() > 15 && !has_dylovene && !patient.reagents.has_reagent(/datum/reagent/medicine/sulfasalazine))
		return TRUE
...
/datum/scanner_advice/damage/oxy/can_show(mob/living/carbon/human/patient, mob/user)
	if(patient.getOxyLoss() > 25 && !patient.reagents.has_reagent(/datum/reagent/medicine/dexalinplus) && !patient.reagents.has_reagent(/datum/reagent/medicine/sulfasalazine))
		return TRUE
...
/datum/scanner_advice/damage/oxy/get_data(mob/living/carbon/human/patient, mob/user)
	. = list(
		...
		ADVICE_TOOLTIP = "If you don't have Dexalin, CPR or treating their other symptoms and waiting for Oxy damage to go down will work.",
...
/datum/scanner_advice/damage/brain/can_show(mob/living/carbon/human/patient, mob/user)
	...
	if(isnull(brain))
		return FALSE
	if(brain.organ_status != ORGAN_HEALTHY && brain.organ_status != ORGAN_BROKEN && !patient.reagents.has_reagent(/datum/reagent/medicine/alkysine) && !patient.reagents.has_reagent(/datum/reagent/medicine/sulfasalazine))
		return TRUE
...
/datum/scanner_advice/damage/brain/critical/can_show(mob/living/carbon/human/patient, mob/user)
	var/datum/internal_organ/brain/brain = patient.get_organ_slot(ORGAN_SLOT_BRAIN)
	if(isnull(brain))
		return FALSE
	if(brain.organ_status == ORGAN_BROKEN && !patient.reagents.has_reagent(/datum/reagent/medicine/sulfasalazine))
		return TRUE

/datum/scanner_advice/damage/brain/critical/get_data(mob/living/carbon/human/patient, mob/user)
	. = list(
		ADVICE_TEXT = "Brain surgery required.",
		ADVICE_TOOLTIP = "Critical brain damage detected, surgical intervention advised.",
		ADVICE_ICON = FA_ICON_BRAIN,
		ADVICE_ICON_COLOR = "red",
	)
...
/datum/scanner_advice/damage/eyes/can_show(mob/living/carbon/human/patient, mob/user)
	...
	if(isnull(eyes))
		return FALSE
	if(eyes.organ_status != ORGAN_HEALTHY && eyes.organ_status != ORGAN_BROKEN && !patient.reagents.has_reagent(/datum/reagent/medicine/imidazoline) && !patient.reagents.has_reagent(/datum/reagent/medicine/sulfasalazine))
		return TRUE
...
/datum/scanner_advice/damage/eyes/critical/can_show(mob/living/carbon/human/patient, mob/user)
	var/datum/internal_organ/eyes/eyes = patient.get_organ_slot(ORGAN_SLOT_EYES)
	if(isnull(eyes))
		return FALSE
	if(eyes.organ_status == ORGAN_BROKEN && !patient.reagents.has_reagent(/datum/reagent/medicine/sulfasalazine))
		return TRUE

/datum/scanner_advice/damage/eyes/critical/get_data(mob/living/carbon/human/patient, mob/user)
	. = list(
		ADVICE_TEXT = "Eye surgery required.",
