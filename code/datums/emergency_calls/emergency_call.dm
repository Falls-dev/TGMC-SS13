...
/datum/emergency_call/proc/activate(announce = TRUE)
	...
	if(announce)
		priority_announce("Сигнал бедствия запущен. Ожидание ответа...", title = "Сигнал Бедствия", type = ANNOUNCEMENT_PRIORITY, sound = 'modular_unga/ru_translate/ru_announce/sound/distressbeacon.ogg', color_override = "orange")
...
/datum/emergency_call/proc/do_activate(announce = TRUE)
	...
	if(mob_min && length(valid_candidates) < mob_min)
		...
		if(announce)
			priority_announce("Ответа на сигнал бедствия не поступило. Системы запуска заняты перекалибровкой.", "Сигнал Бедствия", sound = 'modular_unga/ru_translate/ru_announce/sound/distressbeacon_none.ogg')
	...
	if(announce)
		priority_announce(dispatch_message, "Сигнал Бедствия", sound = 'modular_unga/ru_translate/ru_announce/sound/distressreceived.ogg')
