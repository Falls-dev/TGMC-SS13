...
/datum/round_event/supply_drop/proc/set_target(turf/target_turf)
	...
	priority_announce("Союзный груз прибудет через [drop_delay / 600] минут. Место посадки - [target_turf.loc].", "Статус Тактического Блюспейс Сканера", sound = 'modular_unga/ru_translate/ru_announce/sound/sup_drop.ogg', receivers = (GLOB.alive_human_list_faction[supplying_faction] + GLOB.observer_list))
...
/datum/round_event/supply_drop/proc/alert_hostiles(turf/target_turf, supplying_faction)
	...
	priority_announce("На подходе груз [supplying_faction]. Прибудет через [(drop_delay - alert_delay) / 600] минут. Место - [target_turf.loc].", "Статус Тактического Блюспейс Сканера", sound = 'modular_unga/ru_translate/ru_announce/sound/sup_drop_enemy.ogg', receivers = (humans_to_alert + GLOB.observer_list))


///deploys the actual supply drop
/datum/round_event/supply_drop/proc/drop_supplies(turf/target_turf, faction)
	priority_announce("Обнаружена поставка от [faction] в [target_turf.loc].", "Статус Тактического Блюспейс Сканера", sound = 'modular_unga/ru_translate/ru_announce/sound/sup_drop_act.ogg', receivers = (GLOB.alive_human_list + GLOB.observer_list))
